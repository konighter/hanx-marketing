

# SPCampaignUpdate


## Properties

| Name | Type | Description | Notes |
|------------ | ------------- | ------------- | -------------|
|**budgets** | [**List&lt;SPCreateBudget&gt;**](SPCreateBudget.md) | The object containing budget details for the campaign (for campaigns that support multiple budgets). |  [optional] |
|**campaignId** | **String** | A unique identifier for a campaign. |  |
|**endDateTime** | **OffsetDateTime** | The end date time for the campaign. |  [optional] |
|**name** | **String** | The name of the campaign. |  [optional] |
|**optimizations** | [**SPUpdateCampaignOptimizations**](SPUpdateCampaignOptimizations.md) |  |  [optional] |
|**portfolioId** | **String** | The ID of the portfolio associated with the campaign. |  [optional] |
|**siteRestrictions** | **List&lt;SPSiteRestriction&gt;** | Restrict the ad to a particular site |  [optional] |
|**startDateTime** | **OffsetDateTime** | The start date time for the campaign. |  [optional] |
|**state** | **SPUpdateState** |  |  [optional] |
|**tags** | [**List&lt;SPCreateTag&gt;**](SPCreateTag.md) | Open ended labels with a key value pair applied to the campaign |  [optional] |



