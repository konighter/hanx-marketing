package com.hzltd.module.erplus.service.purchase;
...
public class ErpPurchaseOrderServiceImpl implements ErpPurchaseOrderService {
    ...
    public void updatePurchaseOrderStatus(Long id, Integer status) {
        ...
        // 1.3 存在采购入单，无法反审核
        if (!approve && purchaseOrder.getInCount() != null && purchaseOrder.getInCount() > 0) {
            throw exception(PURCHASE_ORDER_PROCESS_FAIL_EXISTS_IN);
        }
        // 1.4 存在采购退货单，无法反审核
        if (!approve && purchaseOrder.getReturnCount() != null && purchaseOrder.getReturnCount() > 0) {
    ...
    public void updatePurchaseOrderInCount(Long id, Map<Long, BigDecimal> inCountMap) {
        ...
        orderItems.forEach(item -> {
            ...
            if (inCount.equals(item.getInCount())) {
                return;
            }
        ...
        purchaseOrderMapper.updateById(new ErpPurchaseOrderDO().setId(id).setInCount(totalInCount.intValue()));
    }
    ...
    public void updatePurchaseOrderReturnCount(Long orderId, Map<Long, BigDecimal> returnCountMap) {
        ...
        purchaseOrderMapper.updateById(new ErpPurchaseOrderDO().setId(orderId).setReturnCount(totalReturnCount.intValue()));
    }
