...
import com.hzltd.module.erplus.dal.mysql.spu.ErpProductPropertyMapper;
...
public class ProductPropertyServiceImpl implements ProductPropertyService {
    ...
    @Resource
    private ErpProductPropertyMapper erpProductPropertyMapper;
    @Resource
    @Lazy // 延迟加载，解决循环依赖问题
    ...
    public Long createProperty(ProductPropertySaveReqVO createReqVO) {
        // 如果已经添加过该属性项，直接返回
        ProductPropertyDO dbProperty = erpProductPropertyMapper.selectByName(createReqVO.getName());
        if (dbProperty != null) {
        ...
        erpProductPropertyMapper.insert(property);
        // 返回
    ...
    public void updateProperty(ProductPropertySaveReqVO updateReqVO) {
        ...
        // 校验名字重复
        ProductPropertyDO property = erpProductPropertyMapper.selectByName(updateReqVO.getName());
        if (property != null &&
        ...
        erpProductPropertyMapper.updateById(updateObj);
        // 更新 sku 相关属性
    ...
    public PageResult<ProductPropertyDO> getPropertyPage(ProductPropertyPageReqVO pageReqVO) {
        return erpProductPropertyMapper.selectPage(pageReqVO);
    }

    @Override
    public ProductPropertyDO getProperty(Long id) {
        return erpProductPropertyMapper.selectById(id);
    }

    @Override
    public List<ProductPropertyDO> getPropertyList(Collection<Long> ids) {
        return erpProductPropertyMapper.selectBatchIds(ids);
    }
