...
public class ProductSkuServiceImpl implements ProductSkuService {
    ...
    @Resource
    private ErpProductSkuMapper erpProductSkuMapper;
    ...
    public void deleteSku(Long id) {
        ...
        // 删除
        erpProductSkuMapper.deleteById(id);
    }

    private void validateSkuExists(Long id) {
        if (erpProductSkuMapper.selectById(id) == null) {
    ...
    public ProductSkuDO getSku(Long id) {
        return erpProductSkuMapper.selectById(id);
    }
    ...
    public List<ProductSkuDO> getSkuList(Collection<Long> ids) {
        ...
        return erpProductSkuMapper.selectBatchIds(ids);
    }
    ...
    public void createSkuList(Long spuId, List<ProductSkuSaveReqVO> skuCreateReqList) {
        ...
        erpProductSkuMapper.insertBatch(skus);
    }

    @Override
    public List<ProductSkuDO> getSkuListBySpuId(Long spuId) {
        return erpProductSkuMapper.selectListBySpuId(spuId);
    }
    ...
    public List<ProductSkuDO> getSkuListBySpuId(Collection<Long> spuIds) {
        ...
        return erpProductSkuMapper.selectListBySpuId(spuIds);
    }

    @Override
    public void deleteSkuBySpuId(Long spuId) {
        erpProductSkuMapper.deleteBySpuId(spuId);
    }

    @Override
    public int updateSkuProperty(Long propertyId, String propertyName) {
        // 获取所有的 sku
        List<ProductSkuDO> skuDOList = erpProductSkuMapper.selectList();
        // 处理后需要更新的 sku
        ...
        erpProductSkuMapper.updateBatch(updateSkus);
    ...
    public void updateSkuList(Long spuId, List<ProductSkuSaveReqVO> skus) {
        // 构建属性与 SKU 的映射关系;
        Map<String, Long> existsSkuMap = convertMap(erpProductSkuMapper.selectListBySpuId(spuId),
        ...
        if (CollUtil.isNotEmpty(insertSkus)) {
            erpProductSkuMapper.insertBatch(insertSkus);
        }
        if (CollUtil.isNotEmpty(updateSkus)) {
            updateSkus.forEach(sku -> erpProductSkuMapper.updateById(sku));
        }
        if (CollUtil.isNotEmpty(existsSkuMap)) {
            erpProductSkuMapper.deleteBatchIds(existsSkuMap.values());
        }
    }
    ...
    public void updateSkuStock(ProductSkuUpdateStockReqDTO updateStockReqDTO) {
        ...
        updateStockReqDTO.getItems().forEach(item -> {
            if (item.getIncrCount() > 0) {
                erpProductSkuMapper.updateStockIncr(item.getId(), item.getIncrCount());
            } else if (item.getIncrCount() < 0) {
                int updateStockIncr = erpProductSkuMapper.updateStockDecr(item.getId(), item.getIncrCount());
        ...
        // 更新 SPU 库存
        List<ProductSkuDO> skus = erpProductSkuMapper.selectBatchIds(
