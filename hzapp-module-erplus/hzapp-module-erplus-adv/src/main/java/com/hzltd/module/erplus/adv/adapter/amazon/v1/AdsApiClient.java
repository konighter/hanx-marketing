...
import com.hzltd.module.erplus.adv.adapter.amazon.model.AdvRequest;
import com.hzltd.module.erplus.adv.adapter.amazon.model.AdvResponse;
...
public class AdsApiClient {
    ...
    public AdvResponse<String> get(AdvRequest<?> req, String baseUrl) {
        HttpRequest request = buildRequest(req, baseUrl)
                .GET()
                .build();
        return AdvResponse.success(executeRequest(request, baseUrl));
    }
    ...
    public AdvResponse<String> post(AdvRequest<?> req, String baseUrl) {
        HttpRequest request = buildRequest(req, baseUrl)
                .POST(HttpRequest.BodyPublishers.ofString(JsonUtils.toJsonString(req)))
                .build();
...
//                .header("Amazon-Ads-ClientId", clientId)
    ...
    private HttpRequest.Builder buildRequest(AdvRequest<?> req, String baseUrl) {
        String clientId = Optional.ofNullable(configService.getConfigByKey(AMZ_CLIENT_ID))
                .map(ConfigDO::getValue)
                .orElseThrow(() -> new AmazonAdsApiException(0, "Missing config: " + AMZ_CLIENT_ID, baseUrl));

        String url = "https://" + req.getRegion().getAdsEndpoint() + baseUrl;
        HttpRequest.Builder builder = HttpRequest.newBuilder()
                .uri(URI.create(url))
                .timeout(Duration.ofSeconds(30))
//                .header("Content-Type", "application/json")
//                .header("Accept", "application/json")
                .header("Authorization", "Bearer " + req.getCredential().getAccessToken())
//                .header("Amazon-Ads-ClientId", clientId)
                .header("Amazon-Advertising-API-ClientId", clientId);
        if (MapUtils.isNotEmpty(req.getHeaders())) {
            req.getHeaders().forEach((key, value) -> builder.header((String) key, (String) value));
        }

        // 广告账户 ID
        if (StringUtils.isNotEmpty(req.getAccountId())) {
            builder.header("Amazon-Ads-AccountId", req.getAccountId());
        }
        // Profile ID（站点维度）
        if (StringUtils.isNotEmpty(req.getProfileId())) {
            builder.header("Amazon-Advertising-API-Scope", req.getProfileId());
        }
