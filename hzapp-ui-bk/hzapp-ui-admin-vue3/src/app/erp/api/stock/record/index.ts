...
export const StockRecordApi = {
  ...
  getStockRecordPage: async (params: any) => {
    return await request.get({ url: `/erplus/stock-record/page`, params })
  },

  // 查询产品库存明细详情
  getStockRecord: async (id: number) => {
    return await request.get({ url: `/erplus/stock-record/get?id=` + id })
  },

  // 导出产品库存明细 Excel
  exportStockRecord: async (params) => {
    return await request.download({ url: `/erplus/stock-record/export-excel`, params })
  }
}
