<template>
  <el-drawer v-model="visible" size="90%" :destroy-on-close="true" class="shipment-config-drawer">
    <template #header>
      <div class="flex items-center justify-between w-full pr-10">
        <span class="text-lg font-bold flex items-center gap-2 shrink-0">
          ...
          {{ configTitle }}
          <el-button
            v-if="shipmentData.status !== ShipmentStatus.CANCELED && shipmentData.status !== ShipmentStatus.SHIPPED"
            type="danger" link size="small" @click="handleCancel">
...
import { selectComponent } from './componentsSelect'
...
const steps = [
  ...
  { title: '发货', status: ShipmentStatus.PENDING_DELIVERY }
]
...
const activeStep = computed(() => {
  const status = shipmentData.value.status
  switch (status) {
    ...
    case ShipmentStatus.PENDING_DELIVERY: return 4
...
const nextStatus = computed(() => {
  ...
  switch (status) {
    ...
    case ShipmentStatus.PENDING_LABEL: return ShipmentStatus.PENDING_DELIVERY
    case ShipmentStatus.PENDING_DELIVERY: return ShipmentStatus.SHIPPED
...
const handleNext = () => {
  ...
  // 更新shipment状态
  shipmentData.value.status = status
  // 更新选中步骤到新的当前步骤
...
const handleCancel = async () => {
  try {
    ...
    selectedStep.value = 0 // Reset or handle as needed
  } catch (error) {
    if (error !== 'cancel') {
