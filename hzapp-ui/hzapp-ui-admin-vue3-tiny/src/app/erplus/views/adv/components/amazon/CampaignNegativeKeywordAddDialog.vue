<template>
  <el-dialog v-model="visible" title="添加活动级否定关键词" width="800px" append-to-body>
...
const submitKeywords = async () => {
  ...
  try {
    await AmzAdvCampaignManagerApi.batchCreateNegativeKeyword({
      shopId: props.shopId,
    ...
    ElMessage.success('否定关键词添加成功')
    visible.value = false
    emit('success')
  } catch (e) {
    ElMessage.error('否定关键词添加失败')
  } finally {
