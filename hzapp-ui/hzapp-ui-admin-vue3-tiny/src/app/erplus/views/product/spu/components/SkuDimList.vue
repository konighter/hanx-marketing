...
v-for="(item, index) in tableHeaders" :key="index" :label="item.label" align="center"
    ...
    <el-table-column align="center" label="净品规格" min-width="168">
      <template #default="{ row }">

        <el-space :size="56">
          <div class="el-input-group-thir">
            <el-input v-model="row.pkgdim.length" placeholder="长" />
            <el-input v-model="row.pkgdim.width" placeholder="宽" />
            <el-input v-model="row.pkgdim.height" placeholder="高">
              <template #append>cm</template>
            </el-input>
          </div>
          <el-input v-model="row.pkgdim.weight" placeholder="重">
